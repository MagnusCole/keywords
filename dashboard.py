...
st.set_page_config(
    page_title="Keyword Finder Dashboard",
    page_icon="🔍",
    layout="wide"
)
...
class KeywordDashboard:
    ...
    @st.cache_data
    def _get_filtered_keywords(_self, min_score, max_competition, limit):
        filters = {
            'min_score': min_score,
            'limit': limit
        }
        keywords = _self.db.get_keywords(**filters)
        
        # Filtrar por competencia
        filtered = [
            kw for kw in keywords 
            if kw.get('competition', 1) <= max_competition
        ]
        ...
        with col2:
            if st.button("📕 Exportar a PDF"):
                pdf_file = self.exporter.export_to_pdf(
                    keywords[:20], 
                    title="Keyword Research Report - Dashboard"
                )

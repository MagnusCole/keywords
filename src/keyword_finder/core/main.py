...
class KeywordFinder:
    ...
    def _print_volume_targeting_summary(
        ...
        logger.info("Volume targeting summary")
        ...
        if target_filtered > 0:
            ...
            logger.info(
                f"Filtered keywords: {status} {filtered_count:,} (target: {target_filtered:,})"
            )
        else:
            logger.info(f"Filtered keywords: {filtered_count:,} (no target)")
    ...
    def _setup_logging(self):
        ...
        # Set logging context
        set_log_context(run_id=self.run_id, component="keyword_finder", operation="main_execution")
    ...
    async def find_keywords(  # noqa: C901
        ...
        if include_competitors:
            logging.info("Phase 2: Analyzing competitor keywords - Feature not implemented yet")
        ...
        # Use legacy scoring system
        scored_keywords = []
        for kw in all_keywords:
            score = self.scorer.calculate_score(
                volume=kw.get("volume", 0),
                trend_score=kw.get("trend_score", 0.0),

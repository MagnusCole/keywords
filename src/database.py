...
class KeywordDatabase:
    ...
    def get_keywords(self, limit: Optional[int] = None, min_score: float = 0.0) -> List[Dict]:
    ...
    def get_keyword_by_text(self, keyword_text: str) -> Optional[Dict]:
    ...
    def update_keyword_score(self, keyword_text: str, 
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    f"UPDATE keywords SET {', '.join(updates)} WHERE keyword = ?",
                    params
                )
                conn.commit()
                return True
    ...
    def cleanup_old_keywords(self, days: int = 30) -> int:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    "DELETE FROM keywords WHERE last_seen < ?",
